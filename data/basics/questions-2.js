...
export const questions = [
  ...
  },
  {
    ...
    difficulty: "medium",
    question:
      "開発者が、AWS Lambda関数からDynamoDBテーブルへ項目を書き込む処理を実装しています。関数に必要な権限を与えるための最も適切な方法はどれですか。",
    choices: [
      "必要な権限のポリシーを付与したIAMロールを、関数の実行ロールとして設定する",
      "IAMユーザーを作成し、アクセスキーを関数のコードに直接記述する",
      "関数とDynamoDBを同じVPCに配置して接続を許可する",
      "セキュリティグループでDynamoDBへのアウトバウンド通信を許可する",
    ],
    answerIndexes: [0],
    explanation:
      "正解はAです。Lambda関数が他のAWSサービスへアクセスする権限は、関数に設定するIAMロール(実行ロール)で与えます。一時的な認証情報が自動的に使われるため、キーの管理や漏えいのリスクがありません。Bのようにアクセスキーをコードへ直接記述する方法は、漏えいやローテーション管理の問題があり非推奨です。CのVPC配置はネットワーク経路の構成であり、IAMの権限がなければDynamoDBへのAPI呼び出しは拒否されます。Dのセキュリティグループも同様にネットワーク層の制御であって、APIの実行権限を与えるものではありません。",
    reference:
  ...
  },
  {
    ...
    difficulty: "medium",
    question:
      "ある企業が、途中で中断されても再実行すればよい夜間の画像処理バッチをEC2で実行します。処理は厳密な完了時刻に依存せず、コストを可能な限り抑えたいと考えています。最も適切な購入オプションはどれですか。",
    choices: [
      "オンデマンドインスタンス",
      "リザーブドインスタンス",
      "スポットインスタンス",
    ...
    answerIndexes: [2],
    explanation:
      "正解はCです。スポットインスタンスはAWSの空きキャパシティを大幅な割引価格で利用できる購入オプションで、中断される可能性がある代わりに最も低コストです。中断されても再実行できるバッチ処理は、まさにスポットに適したワークロードです。Aのオンデマンドは需要が読めない場合に柔軟ですが割引がなく、最安の要件に合いません。BのリザーブドインスタンスとDのSavings Plansは、1年または3年の利用を約束して定常的な使用分を割り引く仕組みであり、中断可能な断続的バッチを最安で動かす目的とは異なります。",
    reference:
  ...
  },
  {
    ...
    difficulty: "hard",
    question:
      "ある企業が初めての本番ワークロードをAWSで稼働させます。障害時には24時間365日、電話とチャットで技術サポートへ相談でき、本番システム障害時は1時間以内の応答目標が必要です。要件を満たす最も低コストのサポートプランはどれですか。",
    choices: [
      "デベロッパー",
      "ビジネス",
      "エンタープライズ",
    ...
    answerIndexes: [1],
    explanation:
      "正解はBです。ビジネスプランは24時間365日の電話・チャット・メールによる技術サポートを提供し、本番システム障害には1時間以内の応答目標が設定されているため、要件を満たす最も低コストのプランです。Aのデベロッパーは営業時間内のメールによる技術サポートのみで、電話や24時間対応がありません。Cのエンタープライズは15分以内の応答目標や専任のTAMなどミッションクリティカル向けの内容で要件自体は満たしますが、コストが高く「最も低コスト」に反します。Dのベーシックは無料ですが、技術サポート自体が含まれません。",
