...
export const questions = [
  ...
  },
  {
    ...
    answerIndexes: [1, 3],
    explanation:
      "正解はBとDです。AWS WAFのSQLデータベース用マネージドルールグループは、リクエストに含まれるSQLインジェクションのパターンを検査してブロックします。また、レートベースルールは送信元IPアドレスごとのリクエスト数を追跡し、しきい値を超えたIPを自動的に一時ブロックできるため、単一IPからのスクレイピング対策として有効です。AのネットワークACLはIPアドレスやポートなどレイヤー3/4の情報で判定する機能で、HTTPリクエストの中身は検査できません。CのShield StandardはネットワークレイヤーのDDoS緩和が対象で、SQLインジェクション等のアプリケーション層攻撃は防げず、有効化の操作も不要(自動適用)です。Eのポート限定は既に公開しているWebサービスの通信内容の検査にはならず、どちらの問題も解決しません。",
    reference:
