...
export const questions = [
  ...
  },
  {
    ...
    difficulty: "medium",
    question:
      "あるチームのリード開発者に、自身のチーム用のIAMロールを作成・管理する権限を委譲します。ただし、作成されるロールが持てる権限が、チームに許可された範囲(特定のS3・DynamoDBのみなど)を超えないようにして、権限昇格を防ぐ必要があります。最も適切な仕組みはどれですか。",
    choices: [
      "開発者のIAMユーザーにAdministratorAccessをアタッチし、禁止事項を運用手順書で周知する",
      "アクセス許可の境界(permissions boundary)を使用し、開発者が作成するIAMエンティティが持てる最大アクセス許可を制限する",
      "SCPを開発者のIAMユーザーに直接アタッチして、利用できるサービスを制限する",
      "開発者をIAM管理者グループに追加し、グループのポリシーで作成されるロールの権限を自動的に制限する",
    ],
    answerIndexes: [1],
    explanation:
      "正解はBです。アクセス許可の境界(permissions boundary)は、IAMユーザーやロールが持てる最大のアクセス許可を定める管理ポリシーです。権限委譲時に、委譲先が作成するIAMエンティティへ境界の適用を必須にすると、実際に有効な権限はアイデンティティベースポリシーと境界の共通部分に限定され、範囲を超えた権限昇格を防げます。Aは技術的な強制力がなく誤りです。CのSCPは組織のルート・OU・アカウントにのみアタッチでき、個々のIAMユーザーに直接アタッチできないため誤りです。Dのグループのポリシーは所属ユーザーに権限を付与するもので、そのユーザーが作成する別のロールの権限上限を制限する機能はありません。",
    reference:
  ...
  },
  {
    id: "qb-d4-003",
    domain: 4,
    topic: "CloudTrailログの完全性",
    type: "single",
    difficulty: "medium",
    question:
      "コンプライアンス要件により、AWS CloudTrailがS3バケットに配信した証跡ログファイルが、配信後に変更・削除されていないことを暗号的に証明できる必要があります。最も少ない運用負荷でこの要件を満たす方法はどれですか。",
    choices: [
      "CloudTrailのログファイル整合性検証を有効化し、生成されるダイジェストファイルを使って配信後のログの改ざん・削除の有無を検証する",
      "S3バケットのバージョニングを有効化し、ログファイルの世代を保持することで改ざんを防ぐ",
      "配信されたログファイルを毎日担当者がダウンロードしてハッシュ値を手動で記録・比較する",
      "Amazon GuardDutyを有効化して、ログファイルに対する不正アクセスを検出する",
    ],
    answerIndexes: [0],
    explanation:
      "正解はAです。CloudTrailのログファイル整合性検証を有効にすると、CloudTrailは配信したログファイルのSHA-256ハッシュと署名を含むダイジェストファイルを定期的に生成します。このダイジェストファイルを用いることで、ログが配信後に改ざん・削除されていないかを検証でき、証跡の完全性を証明できます。Bのバージョニングは世代を保持しますが、改ざんの有無を暗号的に証明する仕組みではありません。Cの手動でのハッシュ記録・比較は運用負荷が大きく、ヒューマンエラーの余地も残ります。DのGuardDutyは脅威検出サービスであり、ログファイルの完全性を検証する機能ではありません。",
    reference:
