...
export const questions = [
  ...
  // ===== 第2章: データ保護の設計 — 暗号化とシークレット管理 =====
  {
    ...
    answerIndexes: [2],
    explanation:
      "正解はCです。カスタマーマネージドキーは利用者自身が作成・所有するKMSキーで、キーポリシーの編集による利用者制御、CloudTrailによる使用履歴の監査、ローテーション設定(自動ローテーションの有効化や期間指定)の管理をすべて自社で行えます。AのSSE-S3はキー管理がS3側で完結し、キーポリシーによる制御ができません。BのAWSマネージドキーは使用の監査はできますが、キーポリシーの編集やローテーションの管理は自社で制御できません。Dは独自の鍵管理基盤ではキーの使用履歴がAWS CloudTrailに記録されないため監査要件を満たせず、構築・運用の負荷も大きいため不適切です。",
    reference:
